...
if uploaded_file:
    with st.spinner("Chargement et préparation des données..."):
        ...
        # 🕵️ Trouver la valeur encodée de la classe "Dos"
        attack_label_value = None
        target_column = data.columns[-1]  # la dernière colonne est la classe cible
        ...
        if st.button("🚀 Lancer la Simulation"):
            if attack_label_value is None:
                ...
                with st.spinner("Simulation parallèle en cours..."):
                    ...
                    for i in range(10):
                        ...
                        with ThreadPoolExecutor(max_workers=8) as executor:
                            futures = [executor.submit(simulate_slave, batch_data.copy()) for _ in range(3)]

                            for future in futures:
                                batch_result, nb_attacks, preds, trues = future.result()
                                predicted_labels.extend(preds)
                                true_labels.extend(trues)
                                historique = pd.concat([historique, batch_result], ignore_index=True)
                                total_alerts += nb_attacks
                                st.markdown(f"⚠️ Attaques détectées dans ce sous-lot : **{nb_attacks}**")
