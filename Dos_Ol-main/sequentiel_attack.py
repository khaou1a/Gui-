...
# ❌ Ne PAS utiliser st.set_page_config ici !
# st.set_page_config(page_title="Détection d'Intrusionddddddddddddddddddd", layout="wide")


# Simulation de détection
def run_simulation():
    st.markdown("<h2 style='color:#00ffcc;'>🔐 Détection séquentielle des attaques en temps réel </h2>", unsafe_allow_html=True)
    # Zone de texte pour les logs (à mettre dans la fonction pour éviter des problèmes d'affichage)
    log_zone = st.empty()
    ...
    st.markdown(f"- 📈 Historique des flux : `{historique.shape[0]}` lignes")

    st.markdown(f"- 📦 Historique des flux : `{historique.head()}`")